...
print "  "
print "Authors: Bill Sinko <wsinko@gmail.com>"
print "         Yinglong Miao <yinglong.miao@gmail.com>"
print "\n\
Citations:  \n\
...
2. Miao Y, Sinko W, Pierce L, McCammon JA (2013) Reweighting of accelerated molecular dynamics simulations for free energy calculation. In preparation."
print " "
...
## Set control parameters
    ...
    if args.Xdim:
        ...
        binsX, discX= assignbins([-180,180], args)  ## Default bin size
...
##	output_pmf(pmffile,pmf,binsX)
##   	pmffile = 'pmf-comp1-'+str(args.input)+'.xvg'
##	output_pmf(pmffile,c1,binsX)
##   	pmffile = 'pmf-comp2-'+str(args.input)+'.xvg'
##	output_pmf(pmffile,c2,binsX)
##   	pmffile = 'pmf-comp3-'+str(args.input)+'.xvg'
##	output_pmf(pmffile,c3,binsX)
	...
	for x in range(0,n+1):
	  MCweight=np.add(MCweight,(np.divide(np.power(beta_dV, x), float(scipy.factorial(x)))))
	weights=MCweight
...
def reweight_CE(data,hist_min,binsX,discX,dV,T,fit):
    ...
    nA = np.zeros(nbins) # nA is equivalent to hist here
...
def time_amd(dV,T,dV_avg,binf):
    ...
    for i in range(nf):
        j= binf[i]
	time = time+np.exp(dV_avg[j]/(0.001987*T))
    return time
